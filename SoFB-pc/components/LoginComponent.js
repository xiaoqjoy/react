...
const LineAndWechat = styled.div`
    ...
    display: ${props => (props.isHideWeChat ? 'none' : 'block')};
`;
...
export default class Login extends PureComponent {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.isShow) {
            this.setState({
                ...
                isHideWeChat: nextProps.isHideWeChat
            });
        }
    }
    ...
    _weChat() {
        ...
        getAuth().then(({ data }) => {
            ...
            // );
            if (!result.from) {
                result.from = window.location.href;
            }
            obj.redirect_uri = encodeURI(
                // `http://sz.sofbtest.com/login-success?from=${result.from}`
                `http://shenzhen.sofb.com/login-success?from=${result.from}`
            );
            new WxLogin(obj);
    ...
    render() {
        const {
            state: { isWeChat, phone, code }
        } = this;
        return (
            <DiglogWrapper ref='dialogWrapper' isShow={this.state.showLogin}>
                <LoginOuterBox ref='loginOuterBox' id='LoginOuterBox'>
                    <Close
                    ...
                    <LoginBox id='loginBox'>
                        ...
                        <FormItem id='addGeetest'>
                            <Input
                                ...
                                value={phone}
                                maxLength='11'
                                onChange={e => this._getPhone(e)}
                                onMouseEnter={() => {
                                    this.inputRef.current.focus();
                                }}
                            />
                            <Disabled show={this.state.disabledGeetest}>
                                点击按钮进行验证
                            </Disabled>
                        </FormItem>
                        <FormItem>
                        ...
                        <LineAndWechat isHideWeChat={this.state.isHideWeChat}>
                            <LeftLine />
                            <WeChatLogo
                        ...
                        <Tips>
                            <Explain>登录即表示您已阅读并同意</Explain>
                            <Agreement onClick={() => this._toAgreement()}>
                                《搜房宝用户协议》
                            </Agreement>
                        </Tips>
                        <Message
